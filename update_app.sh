...
for i in web_curr_ver backend_curr_ver;
do
	case ${!i} in
	# condition to check for non-empty only numeric input
		''|*[!.0-9]*) echo -e "Current versions are not recorded in curr_versions file!! Please ensure all versions are recorded in the file. Aborting... \n" && exit 1 ;;
	esac
done
...
if [ -z $web_new_ver ] && [ -z $backend_new_ver ]
then
...
for i in $web_new_ver $backend_new_ver;
do
	case $i in
...
do
	new_ver="$i""_new_ver"
	curr_ver="$i""_curr_ver"
    grep -rl --exclude-dir=.git "V${!curr_ver}" $i/ | xargs sed -i "s/V${!curr_ver}/V${!new_ver}/g"
	temp_ver="$i""_version: "
